...
var cheerioURL = function(urlPath) {
	var responseMEB = function(result,response){
...
if(require.main == module) {
    program
		...
		.option('-u  --url <url_file>', 'Fully qualified url to document', null ,URL_DEFAULT)
        .parse(process.argv);

	var checkJson;
	 if ( process.argv.indexOf("--file")  > -1) {	
		console.error("Doing file processing");
	    checkJson = checkHtmlFile(program.file, program.checks);
	 } else if ( process.argv.indexOf("--url")  > -1) {
		console.error("Doing url processing");
